...
extension RewardGraphView {
    ...
    func drawRewardsGraph() {
        ...
        let spacingBetweensDots = (frameWidth - 2 * padding) / (numberOfSections + 0.5)
        
        let shortSegmentLength = spacingBetweensDots * 0.25
        ...
        let img = renderer.image { ctx in
            ...
            for index in 0...Int((numberOfDots - 1)) {
                let x = padding + shortSegmentLength + (spacingBetweensDots * CGFloat(index))
                dots.append(CGPoint(x: x, y: yOffset))
            }
            
            // Define our line between dots
